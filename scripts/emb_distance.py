...
def parse_args():
    ...
    parser.add_argument("--ckpt", "-c", default=None, help="Checkpoint ID")
...
def extract_dataset_name(s):
    match = re.search(r"(?:.*/)?(_resnet_.*)", s)
...
def main():
    ...
    CKPT_PATH = "log/pull_cube_tool" if not args.on_local else "ckpts/"

    # Find job folder safely
    job_folder = find_job_folder(CKPT_PATH, args.job)
    # dataset_name = extract_dataset_name(job_folder)
    ...
    cfg.gpu_id = 0
    cfg._target_ = "guided_dc.agent.AnomalyAgent"
