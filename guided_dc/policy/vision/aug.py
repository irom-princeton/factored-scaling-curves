...
import guided_dc.utils.tensor_utils as tu
...
class CropRandomizer(nn.Module):
    ...
    def forward(self, inputs):
        if isinstance(inputs, dict):
            return {k: self.forward_in(v) for k, v in inputs.items()}
        else:
        ...
        if self.training:
            if torch.rand(1) < self.p:
                if isinstance(x, dict):
                    return {k: self.forward_single(v) for k, v in x.items()}
                else:
...
if __name__ == "__main__":
    ...
    print(img.shape)
    cj = ColorJitter(hue=0.12, brightness=0.2, contrast=0.2, saturation=0.2, p=0.8)
    cc = CropRandomizer(
